...
func init() {
	...
	// 自定义规则 not_exists_in_user, 验证请求数据必须不存在于该用户中
	govalidator.AddCustomRule("not_exists_in_user", func(field string, rule string, message string, value interface{}) error {
		rng := strings.Split(strings.TrimPrefix(rule, "not_exists_in_user:"), ",")
		...
		fmt.Println(userID, "exceptID", exceptID)

		// 拼接 SQL
		query := database.DB.Table(tableName).Where(dbFiled+" = ? AND user_id = ? ", requestValue, userID)
