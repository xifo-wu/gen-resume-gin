...
var CmdMakeAPIController = &cobra.Command{
	Use:   "api-controller",
	Short: "Create api controller, example: make api-controller v1/user",
...
func runMakeAPIController(cmd *cobra.Command, args []string) {
	...
	// 组建目标目录
	filePath := fmt.Sprintf("app/controllers/api/%s/%s_controller.go", apiVersion, model.TableName)

	// 基于模板创建文件（做好变量替换）
	createFileFromStub(filePath, "api_controller", model)
}
